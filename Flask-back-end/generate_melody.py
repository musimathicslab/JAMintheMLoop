...
def midi_input_cleanup(midi_file_path):
...
def generate_continuation(midi_file_path):
  ...
  predicted = estimator.predict(
      input_fn,
      checkpoint_path=ckpt_path #Path of a specific checkpoint to predict.
  )

  # Burn
  next(predicted)
      
  primer_ns = midi_input_cleanup(midi_file_path)
  ...
  decode_length =  256 #Reduced decode_length to the size of target for JAMINtheMLoop purposes.
  if len(targets) >= 4096:
